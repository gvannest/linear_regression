...
class Interval:
	...
	def graphs_residual(self):
		...
		def graph_scatter(ax):
			...
			ax.scatter(self.features[:,-1], self.residuals, c='blue')
			ax.plot(np.array([0, 1.2 * np.max(self.features[:,-1], axis=0)]), np.zeros(2), '-', color="black", alpha=0.8)
		...
		def plot_with_interval(ax):
			...
			ax.plot(x_reg, y_reg, '-', alpha=0.8)

			upper_bound = y_reg + self.prediction_interval
			lower_bound = y_reg - self.prediction_interval
			ax.fill_between(x_reg, upper_bound, lower_bound, color='orange', alpha='0.1')
			ax.legend(loc='upper right')
